      ...
      <div class="sec1">
            ...
            <input type="hidden" id="unq" name="unq" value="${vo.unq }">
                <table id="contentTable" align="center" height="400px;" border="1" style="width: 800px;">
                   <tr>
                       <th width="20%" >제목</th>
                       <td align="left"><input type="text" id="title" name="title" value="${vo.title }"></td>
                   </tr>
                   <tr>
                       <th>일정</th>
                       <td align="left">
                        <select id="year" name="year">
							<c:forEach var="i" begin="2020" end="2030">
								<option value="${i }"<c:if test="${i == vo.year }">selected</c:if>>${i }</option>
						...
						<select id="month" name="month">	  
                           	<c:forEach var="i" begin="1" end="12">
								<option value="${i }"<c:if test="${i == vo.month }">selected</c:if>>${i }</option>
							</c:forEach>  
						</select>
						<span>월</span>
						<select id="day" name="day">
                          	<c:forEach var="i" begin="1" end="31">
							<option value="${i }"<c:if test="${i == vo.day }">selected</c:if>>${i }</option>
						</c:forEach> 
                        </select>
                        <span>일</span>
                   ...
                   <tr>
                       <th>내용</th>
                       <td height="400px;">
                       <textarea id="content" name="content" rows="5" required><c:out value="${vo.content}" escapeXml="false" /></textarea></td>
                   </tr>
                   <tr>
				        <th>파일</th>
				        <td>
				            <input type="file" name="file1" id="file1" style="width:50%" class="input1">
				        </td>
				    </tr>
                ...
                <button type="button" id="btn_submit" onclick="return false;" style="margin-top: 15px;">수정하기</button>
                <button type="button" id="btn_delete" onclick="return false;" style="margin-top: 15px;">삭제하기</button>
